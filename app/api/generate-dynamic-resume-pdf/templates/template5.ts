  ...
  for (let i = 0; i < bodyLines.length; i++) {
    ...
    if (line.endsWith(':')) {
      ...
      if (isJobExperience) {
        ...
        const degreeLines = wrapText(educationEntry.degree, fontBold, bodySize + 1, contentWidth);
        for (const degreeLine of degreeLines) {
          ...
          context.page.drawText(degreeLine, { x: left, y, size: bodySize + 1, font: fontBold, color: BLACK });
          y -= bodyLineHeight + 2;
        }
        
        const institutionLine = `${educationEntry.institution} | ${educationEntry.year}`;
        const institutionLines = wrapText(institutionLine, font, bodySize, contentWidth);
        for (const instLine of institutionLines) {
          ...
          context.page.drawText(instLine, { x: left, y, size: bodySize, font, color: MEDIUM_GRAY });
          y -= bodyLineHeight;
        }
