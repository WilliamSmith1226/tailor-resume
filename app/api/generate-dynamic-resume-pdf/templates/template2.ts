...
import { TemplateContext, wrapText, wrapTextWithIndent, formatDate, drawTextWithBold, COLORS, parseEducationLine, isEducationSection } from '../utils';
...
export async function renderTemplate2(context: TemplateContext): Promise<Uint8Array> {
  ...
  for (let i = 0; i < mainContent.length; i++) {
    ...
    if (line.endsWith(':')) {
      y -= 12;
      const sectionHeader = line.slice(0, -1);
      inEducationSection = isEducationSection(sectionHeader);
      ...
      const isJobExperience = / at .+:.+/.test(line);
      const educationEntry = inEducationSection ? parseEducationLine(line) : null;
      ...
      if (isJobExperience) {
        ...
        // Render education entry professionally
        const degreeLines = wrapText(educationEntry.degree, fontBold, BODY_SIZE + 1, MAIN_WIDTH - 10);
        for (const degreeLine of degreeLines) {
          ...
          context.page.drawText(degreeLine, { x: MAIN_LEFT + 10, y, size: BODY_SIZE + 1, font: fontBold, color: BLACK });
          y -= bodyLineHeight + 2;
        }
        
        const institutionLine = `${educationEntry.institution} | ${educationEntry.year}`;
        const institutionLines = wrapText(institutionLine, font, BODY_SIZE, MAIN_WIDTH - 10);
        for (const instLine of institutionLines) {
          if (y < MARGIN_BOTTOM) {
            context.page = pdfDoc.addPage([PAGE_WIDTH, PAGE_HEIGHT]);
            drawSidebar(context.page, false);
            y = PAGE_HEIGHT - 72;
          }
          context.page.drawText(instLine, { x: MAIN_LEFT + 10, y, size: BODY_SIZE, font, color: MEDIUM_GRAY });
          y -= bodyLineHeight;
        }
