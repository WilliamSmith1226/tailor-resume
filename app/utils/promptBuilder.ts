...
8. CRITICAL SKILLS SECTION: Create an EXCEPTIONALLY RICH, DENSE, and COMPREHENSIVE Skills section. Extract and list EVERY technology, tool, framework, library, service, and methodology from BOTH the JD AND candidate's experience. Make it so comprehensive it dominates keyword matching.
  ...
  8d. FORMATTING: Each skill category MUST be on its own line and MUST start with "• " (bullet character + space). Example:
...
Output the improved resume as plain text, exactly following the original resume's format—including the unchanged headline at the top. Clearly label sections (Summary, Professional Experience, Skills, Education, etc) with original spacing and section order.

CRITICAL FORMATTING REQUIREMENTS:
- Every bullet point in Experience sections MUST start with "• " (bullet character + space) on its own line
- Every skill category in Skills section MUST start with "• " (bullet character + space) on its own line
- Do NOT use numbered lists, plain text paragraphs, or other formats for bullets
- Example Experience bullet: "• Built RESTful APIs using Node.js and PostgreSQL"
- Example Skills line: "• Frontend: React, TypeScript, Next.js, Vue.js"`;
}
